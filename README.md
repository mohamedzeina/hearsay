...
- Top/New sort tabs as a pill segmented control (star + clock icons); Top sorts by upvote count. **Following** appears as a third pill for signed-in viewers — surfaces posts from topics the viewer follows (newest first), with a dedicated "Nothing in your follows yet" empty state that nudges back to the topic pages.
...
A four-layer test pyramid covers utilities, components, queries/actions, and full browser flows. **387 tests** total (+ 5 E2E).
