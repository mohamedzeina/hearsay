...
### Feed scope nav
- **"Following" is a scope, not a sort.** The home feed splits two independent axes: *sort* (Top / New, lives in component state) and *scope* (Everywhere / Following, lives in URL + client state). They render in different controls so the pattern can't be mistaken for a three-way sort.
- **Sidebar "Your feed" panel** (`src/components/feed-nav/feed-nav.tsx`) — `SurfacePanel` with two vertical rows ("Everywhere" / "Following"), each row showing a label + hint ("All of Hearsay" / "Topics you follow"). The active row gets a persimmon left rail (`border-l-2 border-persimmon`) and a cream-2 wash; a small `bg-persimmon-soft text-persimmon-deep` count chip sits next to "Following" when the viewer follows ≥ 1 topic. Rows are `<button>` elements that update scope through the `ScopeProvider`, not `<Link>` — clicking does not trigger a Next router navigation.
...
- **Twitter-style instant swap**: both feeds are fetched on the server **once** in parallel (`fetchRecentPosts` + `fetchFollowingPosts` + the viewer's `topicFollow.count`) and handed to a client `ScopeProvider` that owns the active scope. Toggling between Everywhere and Following is a synchronous React state update — no server roundtrip, no recompile. The URL is mirrored via `window.history.replaceState`; a `popstate` listener keeps the browser back/forward buttons toggling scope correctly. `paths.home({ view })` still returns `/` or `/?view=following` for deep-links.
- **Initial scope** is seeded from `searchParams.view` on the server (Promise-typed in Next 15) so a refresh or direct visit to `/?view=following` lands on the correct tab; thereafter the page server component does not depend on `searchParams`, which is why `replaceState` doesn't trigger a re-render.
- **Sort is sticky across scope swaps**. If you flip from Top to New on Everywhere and then click Following, your sort choice survives the scope change. `PostFeed` shares a single instance across scopes; the `resetKey={scope}` prop snaps pagination back to page 1 when the underlying array changes so swapping into a shorter Following feed never lands you on an empty page.
- **Per-scope copy + empty state**: Following shows "From topics you follow / Scoped to your follows" and a dedicated "Nothing in your follows yet" card with a Browse-topics CTA; Everywhere uses the standard sort-derived header.
- **`PostFeed` is scope-agnostic**: only Top/New sort pills remain in the component; the page passes `title` / `subtitle` / `emptyState` / `resetKey` per scope through a small `<ScopedPostFeed>` client wrapper.
- **Signed-out viewers don't see the FeedNav at all** (both desktop and mobile variants gate on `viewerId`); a stray `?view=following` deep-link silently falls back to Everywhere via the same initial-scope logic (no server redirect, no client redirect — just a calm fallback).
...
- **Editorial save toast** — every save / unsave fires a small index-card toast in the bottom-right (`bg-surface border border-rule shadow-lift rounded-2xl`) with a persimmon dot prefix, a mono uppercase eyebrow ("Saved" / "Unsaved"), a display-font sentence ("Tucked away in /saved." / "Removed from /saved."), and an **Undo** affordance. Auto-dismisses at 3.5s; hover pauses the dismiss clock; `Escape` closes; calling `show()` again *replaces* the active toast instead of stacking a queue. Driven by a new generic `<ToastProvider>` mounted in `src/app/providers.tsx`.
- **Undo restores the card on `/saved`** — unsaving from `/saved` drops the card from the list (via the existing `SavedListContext.removePost`); the toast's Undo button calls a new `restorePost` on the same context, backed by a tombstone cache that remembers each removed post and its original index. The card snaps back at its prior slot in the same frame the silent re-save kicks off. The `SavedListProvider` is hoisted above the empty-state conditional so an undo from the last-unsave still has somewhere to call back into.
- **Undo also snaps the toast away** (`dismiss({ instant: true })`) instead of riding the 220ms fade-out, since the click action itself is the news.
- **Bookmark icon flip is on the urgent lane** — `SaveButton` no longer uses `useOptimistic` because its setter must run inside `startTransition`, which scheduled the icon swap one frame behind the urgent toast render. A manual `pending ?? confirmed` overlay set outside the transition means the icon lands in the same frame as the toast on the home feed (where the icon flip is the only visual feedback). Auto-revert on server failure works the same way: clear the override, the icon snaps back to `confirmed`.
...
- **Header toggle** (sun/moon icon, cross-fade) — `aria-pressed` reflects the active theme, persists to `hearsay:theme` in localStorage.
- **No light-flash** for dark-preference users. An inline `themeBootScript` injected into `<head>` (via `dangerouslySetInnerHTML` in `layout.tsx`) reads the persisted theme — or falls back to `prefers-color-scheme: dark` — and applies the `.dark` class to `<html>` **before paint**.
- **Whole palette is `rgb(var(--name-rgb) / <alpha-value>)`** in `tailwind.config.ts`, so a single `.dark` class swaps every token while utilities like `bg-ink/40`, `text-cream/80`, `border-persimmon/15` keep working under the variable-driven colors. The dark theme overrides only the `*-rgb` triples in `globals.css`; the derived `--foo` named forms follow at use time.
- **Topic chip palette inverts pair-wise** — each of the 8 tones (terracotta / sage / plum / teal / mustard / periwinkle / rust / rose) has a hand-tuned dark variant: dark-tinted bg, brighter pastel label, slightly brightened dot. Tuned so each chip sits ~50% lighter than the surface card so it reads as a distinct color pocket against the dark.
- **Syntax highlighting follows automatically** — the `.hljs-*` rules in `globals.css` already drive off `--ink` / `--persimmon-deep` / `--teal` / `--ink-2` / `--ink-3`, all of which swap. No parallel ruleset needed.
- **Modal backdrops** in `signin-prompt` and `topic-create-form` switched from `bg-ink/40` (which would invert and lighten the page in dark mode) to `bg-black/40 dark:bg-black/60`, so they dim correctly either way.
- All animations + the entire system respect `prefers-reduced-motion`.
...
Every color above is driven by a CSS variable defined in `globals.css` as a space-separated RGB triple (e.g. `--ink-rgb: 26 22 20`). The Tailwind palette references them via `rgb(var(--ink-rgb) / <alpha-value>)` so opacity utilities (`bg-ink/40`, `text-cream/80`, `border-persimmon/15`) keep working under variable-driven colors. A single `.dark` selector overrides only the `*-rgb` triples, which is what makes the [dark theme](#dark-mode) work via one class swap on `<html>`.

Other CSS custom properties (e.g. `--nav-h: 4rem`) live in the same file so the header height has a single source of truth that sidebars can offset from with `top-[calc(var(--nav-h)+2rem)]`.
...
- **`ToastProvider`** (`src/components/common/toast.tsx`) — single-slot transient toast (one visible at a time; `show()` replaces, never stacks). API: `useToast()` returns `{ show, dismiss }`. Payload is `{ eyebrow, body, undo? }` — generic enough for any "action confirmation" surface. Auto-dismiss 3.5s, hover pauses, `Escape` closes, `dismiss({ instant: true })` skips the exit animation. Enter/exit via Tailwind transitions with a `prefers-reduced-motion` fade-only fallback.
- **`ThemeProvider`** (`src/components/theme/theme-provider.tsx`) — owns the active theme and exposes `useTheme()` returning `{ theme, setTheme, toggle }`. `setTheme` writes the `.dark` class on `<html>` and persists to `localStorage` under `hearsay:theme`. The provider also exports `themeBootScript`, a stringified IIFE injected into `<head>` in `layout.tsx` so the theme class lands on the document **before paint** — no light-flash for dark-preference users.
- **`ThemeToggle`** (`src/components/theme/theme-toggle.tsx`) — sun/moon icon button in the header. Cross-fades between icons via opacity (not a swap), full ARIA — `aria-pressed` flips with the theme; the accessible name reads "Switch to dark theme" / "Switch to light theme".
...
- **`FeedNav`** (`src/components/feed-nav/feed-nav.tsx`) — client primitive for the home page's scope switch. Consumes the active scope from `useScope()` and renders a `SurfacePanel` titled "Your feed" with `<button>` rows ("Everywhere" / "Following"). Active row picks up `border-l-2 border-persimmon bg-cream-2/70`; a quiet `bg-persimmon-soft text-persimmon-deep` chip surfaces the viewer's follow count next to "Following" when it's > 0. Mounted only for signed-in viewers.
- **`FeedNavMobile`** (`src/components/feed-nav/feed-nav-mobile.tsx`) — `lg:hidden` horizontal pill strip rendered above the feed on small screens. Same `useScope()` active-state logic as `FeedNav`, same `bg-surface text-ink shadow-soft` active pill as the sort control immediately beneath it.
- **`ScopeProvider`** (`src/components/feed-nav/scope-provider.tsx`) — owns the home page's scope state; mounts between the hero and the grid so both the sidebar `FeedNav` and the in-feed `FeedNavMobile` share the same context. Exposes `useScope()` returning `{ scope, setScope, canFollow }`. `setScope` updates state synchronously and mirrors the URL via `history.replaceState` (not Next's router) so the swap doesn't trigger a server re-render. A `popstate` listener keeps the active scope synced with browser back/forward.
- **`ScopedPostFeed`** (`src/components/feed-nav/scoped-post-feed.tsx`) — thin client wrapper that consumes the current scope and renders `PostFeed` with the matching array + per-scope `title` / `subtitle` / `emptyState` / `resetKey`. Single `PostFeed` instance across scopes so the active sort stays sticky.
...
A four-layer test pyramid covers utilities, components, queries/actions, and full browser flows. **446 tests** total (+ 5 E2E).
...
| Component | Vitest + React Testing Library | Mocked NextAuth/router; covers Avatar, FormError/Button, VoteButton, SaveButton (incl. fire-toast-on-save-with-/saved-copy, fire-toast-on-unsave-with-removed-copy, Undo re-toggles silently without a second toast, server-failure dismisses the toast, unauthenticated click skips toast), FollowButton (auth gate, optimistic toggle, server-failure revert, Follow/Following label swap), FeedNav + FeedNavMobile (active-state derived from ScopeProvider context, follow-count chip gated by count > 0, `lg:hidden` wrapper on the mobile fallback, URL sync via `history.replaceState` on click, signed-in-only mount, mobile + sidebar stay in sync as siblings of the same provider), ScopedPostFeed (sticky sort across scope swaps, per-scope title/subtitle/emptyState, swaps arrays without remounting PostFeed), Toast (show/replace/auto-dismiss/Undo callback/Escape/snap-dismiss/`useToast` outside provider throws), ThemeProvider + ThemeToggle (reads initial theme from DOM class, setTheme/toggle DOM-class + localStorage side effects, boot script applies persisted/system theme + survives disabled storage, aria-pressed flips with theme, click cycle persists), CSS-var swap sanity (toggling `.dark` updates resolved RGB on chip tokens), markVisited (most-recent-first, dedupe-on-revisit, no-op at head, MAX cap eviction, disabled-storage survival, corrupt-JSON tolerance), PostCardList visited fade (unvisited at full opacity, visited renders `data-visited` + `opacity-60`, live updates when markVisited fires after mount via the bus), SavedPostsList (unsave-removes-card, Undo restores at original index even when the list emptied, last-card-unsave provider stays mounted), PostFeed (Top/New sort pills only, defaultSort honoured, title/subtitle overrides, custom emptyState injection, fallback PostEmpty when no override), Markdown (incl. hljs token classes on fenced blocks, `@mention` autolinks to `/u/<name>` with internal-link styling, no false-positive on emails, suppressed inside code), SignInPromptProvider, CommentCard (incl. copy-link + clipboard fallbacks), CommentShow (anchor wrapping), CommentListClient (default-new ordering, top vote-count sort with newest-first tiebreak, old reverse, parent-only scoping, empty-state pill hiding, soft-deleted exclusion from the count), NotificationsBell (badge states + 99+ cap, no auto-mark on open, per-row mark-read + decrement, explicit "Mark all read" pill, distinct verbs per kind, "in" prefix for comment-kind rows, comment-anchored deep-links vs post-only links, "See all" footer link to `/notifications`, cross-component bus decrements from list dispatches, live polling on the interval, pause-while-open + pause-while-hidden, immediate refresh on tab-visible, stop-on-unmount), NotificationsList (empty state, pagination at 10/page with prev/next state, per-row mark-read, deep-link href shape, filter pill row presence + per-kind narrowing, filter-specific empty state, page-1 snap on filter change, "Mark all read" scoping vs filtered view, bus dispatch on row + bulk mark-read), MentionTextarea (no trigger pre-`@`, dropdown opens after `@`, prefix-query in URL, email guard, Escape closes, click + Enter insert, ↑/↓ navigate, no-matches hint, portal escape from `overflow-hidden` ancestor), AuthorChip, TopicPostsEmpty (start-the-discussion CTA), CharCounter (min hint, in-range, ≥90% persimmon, over-max), draft restore-from-storage on comment + topic forms (incl. parentId scoping for nested replies), all auth-gated create forms | 197 |
...
│   │   │   └── toast.tsx               # Single-slot transient toast (eyebrow + body + Undo)
│   │   ├── posts/
...
│   │   │   ├── scope-provider.tsx      # Client scope state + URL sync via history.replaceState
│   │   │   └── scoped-post-feed.tsx    # Picks Everywhere vs Following array; passes per-scope copy to PostFeed
│   │   ├── notifications/
...
│   │   ├── theme/
│   │   │   ├── theme-provider.tsx      # Active theme + boot script + localStorage persistence
