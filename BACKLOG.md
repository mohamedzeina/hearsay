...
- ✅ Activity / read-state per post — local-only "you've already read this" hint. Mounting a post detail page calls `markVisited(postId)`, which writes a deduped, most-recent-first list under `hearsay:visited` (capped at 200 entries; corrupt JSON and disabled-storage paths degrade silently). `PostCardList` subscribes via `useVisited()` and fades visited cards to `opacity-60` everywhere it's rendered (home feed, /saved, topic pages, profile, search) — `hover:opacity-100` so a faded card is still inviting. Strictly a visual hint: order is untouched, no server tracking, no algorithmic re-ranking. Same-tab sync via a window CustomEvent; cross-tab via the native storage event.
