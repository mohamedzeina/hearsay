...
- ✅ CI pipeline + coverage gate — `.github/workflows/test.yml` runs the full test pyramid on every PR and push to main: postgres:16-alpine service container (same image + creds + port as `docker-compose.test.yml`), Prisma generate, lint, unit + integration via `npm run test:coverage`, Playwright (chromium with system deps), and a Playwright report artifact uploaded on failure. Concurrent runs of the same branch cancel earlier ones so a fresh push doesn't burn runner minutes. `vitest.config.ts` enforces baseline coverage thresholds (statements/lines 60, branches 80, functions 70 — a few points below the current 66/86/77/77 numbers) so a meaningful drop fails the build but routine edits don't trip on rounding; `@vitest/coverage-v8` added as a dev dep.
- ✅ Activity / read-state per post — local-only "you've already read this" hint. Mounting a post detail page calls `markVisited(postId)`, which writes a deduped, most-recent-first list under `hearsay:visited` (capped at 200 entries; corrupt JSON and disabled-storage paths degrade silently). A shared `<VisitedLi>` client wrapper subscribes via `useVisited()` and fades visited cards to `opacity-60` with `hover:opacity-100` so a faded card is still inviting. Applies everywhere a list of posts renders: home (`PostCardList` → both Everywhere and Following scopes), `/saved`, `/search`, topic pages (`PostList`), user profile, AND the post-detail "More in this topic" sidebar (`RelatedPosts`). `!important` on the visited-state opacity so the fade wins over the `.rise` entry animation's `opacity: 1` end-state. Strictly a visual hint: order is untouched, no server tracking, no algorithmic re-ranking. Same-tab sync via a window `CustomEvent`; cross-tab via the native `storage` event.
...
_CI pipeline + coverage gate shipped. No remaining Tier 6 items._
...
_Last touched 2026-05-23 (after read-state ship). Update or trash as priorities shift._
